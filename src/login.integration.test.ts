...
describe("login integration: standard mode persists credentials to disk", () => {
  ...
  it("refreshes the source_profile when invoked with an AWS CLI assume-role profile", async () => {
    const requestedProfileName = "app-prod-admin";
    const sourceProfileName = "az2aws-source-prod";
    const sourceRoleArn = "arn:aws:iam::123456789012:role/Az2awsSourceRole";
    const targetRoleArn = "arn:aws:iam::123456789012:role/ExampleAdminRole";
    const principalArn = "arn:aws:iam::123456789012:saml-provider/TestProvider";
    ...
    const configIni = [
      ...
      "azure_tenant_id = tenant-xyz",
      "azure_app_id_uri = https://app.example.com",
      `azure_default_role_arn = ${sourceRoleArn}`,
      "azure_default_duration_hours = 1",
      "azure_default_remember_me = false",
      "",
      `[profile ${requestedProfileName}]`,
      `role_arn = ${targetRoleArn}`,
    ...
    vi.spyOn(login, "_parseRolesFromSamlResponse").mockReturnValue([
      { roleArn: sourceRoleArn, principalArn },
    ]);
    vi.spyOn(console, "log").mockImplementation(() => {});
    ...
    expect(console.log).toHaveBeenCalledWith(
      `Use them with AWS CLI by passing --profile "${requestedProfileName}".`,
    );
    ...
    expect(parsed[requestedProfileName]).toBeUndefined();
    const sourceSection = parsed[sourceProfileName];
    expect(sourceSection).toBeDefined();
    expect(sourceSection.aws_access_key_id).toBe("ASIAIOSFODNN7EXAMPLE");
    expect(sourceSection.aws_session_token).toBe("source-session-token");
    expect(sourceSection.aws_expiration).toBe(expectedExpiration.toISOString());
